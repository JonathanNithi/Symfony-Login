...
class sfGuardSecurityUser extends sfBasicSecurityUser
{
  ...
  /**
   * Returns the sfGuardUser object's Employee_Id.
   *
   * @return string
   */
  public function getEmployeeId()
  {
    return $this->getGuardUser()->getEmployeeId();
  }


  /**
   * Returns the sfGuardUser object's Full name.
   *
   * @return string
   */
  public function getFullName($eId)
  {
    $q = Doctrine_Core::getTable('employee')
    ->createQuery('c')
    ->where('c.e_id = ?', $eId);
    $comments = $q->execute();
    return $comments[0]->getFname()." ".$comments[0]->getLname();
  }

  /**
   * Returns the sfGuardUser object's Address.
   *
   * @return string
   */
  public function getAddress($eId)
  {
    $q = Doctrine_Core::getTable('employee')
    ->createQuery('c')
    ->where('c.e_id = ?', $eId);
    $comments = $q->execute();
    return $comments[0]->getAddress();
  }
